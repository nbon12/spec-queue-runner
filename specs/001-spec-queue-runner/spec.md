...
- **FR-012b**: The branch MUST be cut from the base branch as it stands on the remote, fetched immediately beforehand — never from the clone's local ref, whose age nothing establishes. If that fetch fails the worktree MUST NOT be created and the item MUST be left untouched for a later tick; branching from a base of unknown age is not an acceptable degraded mode, because the staleness it introduces travels all the way to the merge.
...
- **FR-033e**: A branch that has fallen behind the base MUST NOT be merged as-is. Before the change is reviewed, and again immediately before it is merged, the runner MUST fetch the base and establish the branch's distance from it; a count it cannot establish MUST be treated as unknown rather than zero. If the base has moved, the branch MUST be rebased onto it and force-pushed with lease, and review and verification MUST be performed on the rebased branch — a verdict reached against a base that has since moved is not evidence about the code that would land. A rebase that conflicts MUST be aborted, reported on the issue, and MUST NOT merge. If the base moves after review and verification have run, the merge MUST be deferred to a later tick rather than performed on the strength of checks that predate the move.
